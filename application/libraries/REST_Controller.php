<?php
	...
	abstract class REST_Controller extends CI_Controller {
		...
		protected function _log_request ( $authorized = FALSE ) {
			...
			$is_inserted = $this->rest->db->insert( $this->config->item( 'rest_logs_table' ), array(
		...
		protected function _log_access_time () {
			...
			return $this->rest->db->update( $this->config->item( 'rest_logs_table' ), $payload, array(
				'id' => $this->_insert_id
			) );
		}
		...
		public function _getUser ( $token ) {
			...
			$access_token = $this->_get_access_token( 'token', $token );
			if ( $access_token ) {
				$dbdate = strtotime( $access_token->date_accessed );
				$customer = $this->_get_member( $access_token->customer_id);
				$customer->verified = ($customer->customer_verified) ? TRUE : FALSE;
				return $customer;
			}
		...
		function _get_access_token ( $field, $value ) {
			$query = $this->rest->db->where( $field, $value )->get( 'access_token' );
			$row = $query->row();
			return (isset( $row )) ? $row : FALSE;
		}

		function _get_member ( $id ) {
			$query = $this->rest->db->where( 'customer_id', $id )->get( 'customers' );
			$row = $query->row();
			return (isset( $row )) ? $row : FALSE;
		}
		...
		function _decipher_activation ( $code ) {
			...
			$delimiter = $this->configs->get( 'encryption_delimiter' );
			$decrypted = $this->encryption->decrypt( urldecode( $code ) );
			list( $account, $dummy ) = explode( $delimiter, $decrypted );
			return $account;
		}


		function _send_email ( $customer, $details = array(), $action = 'send_verification' ) {

			$this->load->library( 'Mailer' );
			$data = array();
			$data['from'] = $this->configs->get( 'mail_sender' );
			$verificationEndpoint = $this->configs->get( 'verification_endpoint' );
			$data['fromName'] = $this->configs->get( 'mail_sender_name' );
			$data['to'] = $customer->email;

			$data['subject'] = 'Email Verification';
			$verificationToken = $this->_generate_verification_token($customer->id );
